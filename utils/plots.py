...
def plot_one_box(x, img, color=None, label=None, line_thickness=2):
    ...
    if str(label.split(' ')[0]) == "佩戴口罩":
        # print("hongse")
        cv2.rectangle(img, c1, c2, (0, 0, 225), thickness=2, lineType=cv2.LINE_AA)

    elif str(label.split(' ')[0]) == "未佩戴口罩":
        cv2.rectangle(img, c1, c2, (0, 166, 256), thickness=2, lineType=cv2.LINE_AA)
    elif str(label.split(' ')[0]) == "未正确佩戴口罩":
